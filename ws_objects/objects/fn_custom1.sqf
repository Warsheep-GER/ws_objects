...
if (ws_custom1 == 0) exitWith {
	if (ws_debug == 0)exitWith {};
	diag_log "WsObjects CUSTOM1 OBJECTS Disabled";
};
...
if (ws_custom1 == 1) exitWith {
	if (ws_debug == 1) then {
		diag_log "WsObjects CUSTOM1 OBJECTS Enabled";
	};
	_objects = 
		[
	...
	forEach _objects;
	if (ws_debug == 0)exitWith {};
	diag_log "WsObjects CUSTOM1 OBJECTS Spawned";
};