...
if (ws_custom2 == 0) exitWith {
	if (ws_debug == 0)exitWith {};
	diag_log "WsObjects CUSTOM2 OBJECTS Disabled";
};
...
if (ws_custom2 == 1) exitWith {
	if (ws_debug == 1) then {
		diag_log "WsObjects CUSTOM2 OBJECTS Enabled";
	};
	_objects = 
		[
	...
	forEach _objects;
	if (ws_debug == 0)exitWith {};
	diag_log "WsObjects CUSTOM2 OBJECTS Spawned";
};